...
class FiltersBot():
                                    ...
                                    match len( upd.callback_query.data.split('.') ) : 
                                        ...
                                        case 6:
                                            range_keyb = self.inline_builder(
                                                [
                                                    [("от",f"{upd.callback_query.data}.min")],
                                                    [("до",f"{upd.callback_query.data}.max")]
                                                ]
                                            )
                                        ...
                                        #case 8 :
                                            #self.bot.edit_message_text( text= f"{try_str(  lang_strings, upd.callback_query.data.split('.')[-1], default=  upd.callback_query.data )}\nchosen year: {upd.callback_query.data.split('.')[7]}" , chat_id=upd.callback_query.message.chat.id, message_id=upd.callback_query.message.message_id, reply_markup= None)               
    ...
    def listing( self, upd, lang_strings, prop, page=0 ):
        ...
        if len( prop["options"][   0 + page*(tg_inline_lim-cb_default)   ::] ) <= tg_inline_lim-control_buttons_number :
            ...
            next_button =  [types.InlineKeyboardButton( text="next", callback_data=f"{upd.callback_query.data}.{page+1}" )]
